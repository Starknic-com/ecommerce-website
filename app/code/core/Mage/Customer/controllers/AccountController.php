...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        Mage::log("Ashok-1", null, "ashokLogin.log");
        $session = $this->_getSession();

        if (!$session->getBeforeAuthUrl() || $session->getBeforeAuthUrl() == Mage::getBaseUrl()) {
            Mage::log("Ashok-2", null, "ashokLogin.log");
            // Set default URL to redirect customer to
            $session->setBeforeAuthUrl($this->_getHelper('customer')->getAccountUrl());
            // Redirect customer to the last page visited after logging in
            if ($session->isLoggedIn()) {
                Mage::log("Ashok-3", null, "ashokLogin.log");
                if (!Mage::getStoreConfigFlag(
                    Mage_Customer_Helper_Data::XML_PATH_CUSTOMER_STARTUP_REDIRECT_TO_DASHBOARD
                )) {
                    $referer = $this->getRequest()->getParam(Mage_Customer_Helper_Data::REFERER_QUERY_PARAM_NAME);
                    Mage::log("Ashok-4-".$referer, null, "ashokLogin.log");
                    if ($referer) {
                        Mage::log("Ashok-5", null, "ashokLogin.log");
                        if (strpos($state, "checkout/onepage") == false) {
                            // Rebuild referer URL to handle the case when SID was changed
                            $referer = $this->_getModel('core/url')
                                ->getRebuiltUrl( $this->_getHelper('core')->urlDecodeAndEscape($referer));
                        }
                        Mage::log("Ashok-6-".$referer, null, "ashokLogin.log");
                        //if ($this->_isUrlInternal($referer)) {
                            Mage::log("Ashok-7", null, "ashokLogin.log");
        ...
        $state = "/";
        if(($referer = $session->getBeforeAuthUrl(true))){
            $state = $referer;
        }
        if (strpos($state, "customer/account") != false) { 
            $state = Mage::getBaseUrl();
        }
        $this->_redirectUrl($state);
