...
class MGS_Mpanel_Helper_Social extends Mage_Core_Helper_Abstract
{
	...
	public function formatTwitString($strTweet){
		...
		$truncateCount = Mage::getStoreConfig('mpanel/twitter/truncate');
		if($truncateCount!='' && $truncateCount>0){
			$strTweet = Mage::helper('core/string')->truncate($strTweet, $truncateCount);
		}
		return $strTweet;
	}
	
	public function getLastTwitter(){
		$tweetUser = Mage::getStoreConfig('mpanel/twitter/twitter_user');
		...
		$count = Mage::getStoreConfig('mpanel/twitter/twitter_count');
			...
			if(!isset($twitter_data['errors'])){
				try{
					if(count($twitter_data)>0){
						$html .= '<div><div class="footer-title"><h4>'.Mage::getStoreConfig('mpanel/twitter/twitter_title').'</h4></div>';
						$html .= '<ul class="twitter_feed">';
						foreach ($twitter_data as $key => $value) {
							$html .= '<li><span></span><p>'.$this->formatTwitString($twitter_data[$key]['text']).' <a href="https://twitter.com/'.$twitter_data[$key]['user']['screen_name'].'/status/'.$twitter_data[$key]['id_str'].'"><p style="font-style: italic; margin-top: 0;">about '.$this->relativeTime($twitter_data[$key]['created_at']).'</p></a></p></li>';
						}
						$html .= '</ul></div>';
					}
				}
