    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        ob_start();
			...
			if($el=='h1' || $el=='h2' || $el=='h3' || $el=='h4' || $el=='h5' || $el=='h6'){
				echo '<'.$el.' id="'.$htmlId.'_view" style="display:block; padding:2px 0 10px 0;">Heading '.str_replace('h','',$el).'</'.$el.'>';
			}
			else{
				if($el=='price'){
					echo '<div class="price-box accent" style="font-size:20px; display:block; padding:2px 0 10px 0;"><span class="price" id="'.$element->getHtmlId().'_view">$289</span></div>';
				}
				else{
					if($el=='menu'){
						echo '<div id="'.$htmlId.'_view" style="font-size:16px; display:block; padding:2px 0 10px 0;">HOME</div>';
					}
					else{
						echo '<span id="'.$element->getHtmlId().'_view" style="font-size:14px; display:block; padding:2px 0 10px 0;">Lorem ipsum dolor sit amet</span>';
					}
				}
			}
        ?>
