					...
					if(isset($file_parts['extension']) && $file_parts['extension']=='xml'){
						...
						if (is_dir($dirHome)) {
							if ($dhHome = opendir($dirHome)) {
								//while (($fileHome = readdir($dhHome)) !== false) {
								...
								foreach ($fileHomes as $fileHome){
									...
									if(isset($file_parts_home['extension']) && $file_parts_home['extension']=='xml'){
										...
										$installUrl = $this->getUrl('mpanel_admin/adminhtml_install/demo', array('theme'=>$theme,'home'=>$home,'website'=>$this->getRequest()->getParam('website'),'store'=>$this->getRequest()->getParam('store')));
										...
										$src = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_WEB).'skin/frontend/mgstheme/'.$theme.'/asset/homes/'.$home.'.jpg';
