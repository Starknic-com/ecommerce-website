...
class Mage_Ccavenuepay_Model_Config
{
    ...
    const CGI_URL = 'https://secure.ccavenue.com/transaction.do?command=initiateTransaction';
    
    const CGI_TEST_URL = 'https://test.ccavenue.com/transaction.do?command=initiateTransaction';
	...
	public function getCcavenuepayServerUrl()
	{
        ...
        $transactionMode = Mage::getStoreConfig('payment/ccavenuepay/transaction_mode', $store);
        
        if($transactionMode == 'LIVE'){
            return self::CGI_URL;
        }else{
            return self::CGI_TEST_URL;
        }
	}
